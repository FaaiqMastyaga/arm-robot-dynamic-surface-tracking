...
# ---------------------------------------------------------
# 1. LIGHTWEIGHT ROS WORKER (Frontend Only)
# ---------------------------------------------------------
class RosFrontendWorker(QThread):
    status_signal = Signal(DashboardStatus)
    ...
    def run(self):
        ...
        self.node = Node('dashboard_gui_client')
        ...
        # --- Subscribers ---
        self.node.create_subscription(DashboardStatus, '/dashboard/status', self.cb_status, 10)
    ...
    def cb_status(self, msg: DashboardStatus):
...
class DrawableCanvas(QWidget):
    def __init__(self, phys_width_m=0.30, phys_height_m=0.20):
        ...
        self.phys_w = phys_width_m; self.phys_h = phys_height_m
        self.target_ratio = self.phys_w / self.phys_h
        self.lines = []; self.current_line = []; self.is_drawing = False
        self.active_rect = QRectF() 
        self.bg_color = QColor(60, 62, 66); self.border_color = QColor(120, 120, 125); self.drawing_color = QColor(255, 255, 255)
    ...
    def resizeEvent(self, event):
        ...
        current_ratio = w / h
        pad = 20
        if current_ratio > self.target_ratio: draw_h = h - pad; draw_w = draw_h * self.target_ratio
        else: draw_w = w - pad; draw_h = draw_w / self.target_ratio
        x = (w - draw_w) / 2.0; y = (h - draw_h) / 2.0
    ...
    def mouseMoveEvent(self, event):
        if self.is_drawing:
            pos = event.position()
            if self.active_rect.contains(pos): self.current_line.append(pos)
            else:
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton: self.is_drawing = False

    def paintEvent(self, event):
        painter = QPainter(self); painter.setRenderHint(QPainter.Antialiasing)
        painter.fillRect(self.active_rect, self.bg_color)
        border_pen = QPen(self.border_color, 3, Qt.DashLine); painter.setPen(border_pen); painter.drawRoundedRect(self.active_rect, 10, 10)
        traj_pen = QPen(self.drawing_color, self.line_thickness, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin); painter.setPen(traj_pen)
    ...
    def get_points(self, hover_height_m=0.01):
        ...
        scale_x = self.phys_w / self.active_rect.width(); scale_y = self.phys_h / self.active_rect.height()
...
# ---------------------------------------------------------
# 3. MAIN WINDOW GUI (Streamlined with all tabs)
# ---------------------------------------------------------
class DashboardClient(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Dynamic Tracking Interface (Client)")
        self.resize(1300, 850)

        # Load YAML
        self.canvas_w = 0.30; self.canvas_h = 0.20
        try:
        ...
        # Connect Signals
        self.worker.status_signal.connect(self.update_ui_from_status)
    ...
    def setup_ui(self):
        ...
        self.result_log = QLineEdit("Client Ready."); self.result_log.setReadOnly(True)
    ...
    def create_canvas_tab(self):
        ...
        self.lbl_action_status = QLabel("Status: Idle"); self.lbl_action_status.setAlignment(Qt.AlignCenter)
    ...
    def create_cartesian_goal_tab(self):
        ...
        v_w, v_s = self.create_velocity_control(); self.vel_sliders_ui.append(v_s); l.addWidget(v_w)
    ...
    def create_right_panel(self):
        ...
        gb_s = QGroupBox("Robot Status (from Server)"); gl_s = QGridLayout()
        self.lbl_servo = QLineEdit("Waiting..."); self.lbl_servo.setReadOnly(True); self.lbl_servo.setAlignment(Qt.AlignCenter)
        self.lbl_fault = QLineEdit("Waiting..."); self.lbl_fault.setReadOnly(True); self.lbl_fault.setAlignment(Qt.AlignCenter)
    ...
    def on_tab_changed(self, index):
        # Index 0 = Canvas (Requires Servo for dynamic tracking)
    ...
    def on_publish_cart(self):
        ...
        f = 'elfin_base_link' if self.frame_group.checkedId() == 0 else 'world'
        self.worker.publish_cart_goal(vals[:3], vals[3:], f)
    ...
    def stop_jog(self):
        ...
        if self.active_jog:
            ...
            self.send_jog_msg(jtype, axis, 0.0, frame)
        self.active_jog = None

    def jog_loop(self):
        if not self.active_jog: return
        jtype, axis, direction, frame = self.active_jog
        self.send_jog_msg(jtype, axis, direction, frame)

    def send_jog_msg(self, jtype, axis, direction, frame):
        msg = JogCommand()
        msg.jog_type = jtype
        msg.axis = int(axis)
        msg.direction = float(direction)
        msg.velocity_scale = float(self.worker.current_vel_scale)
        msg.frame_id = frame
    ...
    # --- UI Updating (Powered by the Server's DashboardStatus message) ---
    @Slot(DashboardStatus)
    def update_ui_from_status(self, msg: DashboardStatus):
        for k, val in zip(["X","Y","Z","Qx","Qy","Qz","Qw"], [msg.x, msg.y, msg.z, msg.qx, msg.qy, msg.qz, msg.qw]): 
            self.ee_disp[k].setText(f"{val:.3f}")
        for i, val in enumerate(msg.joint_positions): 
            self.j_disp_widgets[i].setText(f"{val:.1f}")
        
        self.lbl_servo.setText("ENABLED" if msg.is_enabled else "DISABLED")
        self.lbl_servo.setStyleSheet(f"background-color: {'#90EE90' if msg.is_enabled else '#FFB6C1'}; font-weight: bold;")
        self.lbl_fault.setText("FAULT" if msg.is_faulted else "NO FAULT")
        self.lbl_fault.setStyleSheet(f"background-color: {'#FF4500' if msg.is_faulted else '#90EE90'}; font-weight: bold;")
        
        text = {0: 'Ready', 1: 'No Warning', 2: 'Collision!', 3: 'Collision!', 4: 'Singularity'}.get(msg.servo_status, f"Code: {msg.servo_status}")
        self.lbl_moveit_status.setText(text)
        if msg.servo_status == 0: self.lbl_moveit_status.setStyleSheet("background-color: #90EE90; color: black;")
        elif msg.servo_status in [1, 3]: self.lbl_moveit_status.setStyleSheet("background-color: #FFD700; color: black;")
